(function() {
  ...
  function shareMemo(memoId) {
    try {
      ...
      generateMemoImage(memo, preferredStyle, preferredFont, true).then(imageDataUrl => {
  ...
  function showImagePreviewWithStyleSelector(imageDataUrl, memo, currentStyle) {
    ...
    const html = `
      <div style="padding: 20px 0;">
        ...
        <!-- 是否显示角色照片的选项 -->
        <div style="margin-bottom: 20px; padding: 16px; border: 1px solid var(--SmartThemeBorderColor, rgba(255, 255, 255, 0.1)); border-radius: 8px; background: var(--SmartThemeQuoteColor, rgba(255, 255, 255, 0.05));">
          <div style="display: flex; align-items: center; gap: 12px;">
            <input type="checkbox" id="showCharacterPhoto" checked style="
              width: 18px; 
              height: 18px; 
              cursor: pointer;
              accent-color: var(--SmartThemeQuoteColor, #4a9eff);
            ">
            <label for="showCharacterPhoto" style="
              color: var(--SmartThemeBodyColor, rgba(255, 255, 255, 0.9)); 
              font-size: 14px; 
              cursor: pointer; 
              user-select: none;
          ...
          <div style="margin-top: 8px; font-size: 12px; color: var(--SmartThemeBodyColor, rgba(255, 255, 255, 0.6));">
            取消勾选将隐藏图片左上角的角色照片并优化布局
          </div>
        </div>
    ...
    styleOptions.forEach(option => {
      option.addEventListener('click', () => {
        ...
        const showCharacterPhoto = modalBodyElement.querySelector('#showCharacterPhoto').checked;
        generateImageWithStyle(memo, style, showCharacterPhoto);
      });
    });
  ...
  // 使用指定风格生成图片
  function generateImageWithStyle(memo, style, showCharacterPhoto) {
    // 显示加载提示
    toastr.info('正在生成图片...');
  
    // 生成图片
    generateMemoImage(memo, style, showCharacterPhoto).then(imageDataUrl => {
      // 显示图片预览
  ...
  // 生成Memo图片
  function generateMemoImage(memo, style = 'summer', customFont = 'QiushuiShotai', showCharacterPhoto = true) {
    return new Promise((resolve, reject) => {
      try {
        ...
        const width = 800;
        // 先计算内容所需的高度
        const estimatedHeight = calculateContentHeight(ctx, memo, width, customFont, showCharacterPhoto);
        ...
        // 绘制内容
        drawMemoContent(ctx, memo, width, height, theme, customFont, showCharacterPhoto).then(() => {
          // 返回图片数据
  ...
  // 计算内容所需的高度
  function calculateContentHeight(ctx, memo, width, customFont, showCharacterPhoto = true) {
    const padding = 30;
    const contentWidth = width - padding * 2;
    let totalHeight = padding + 20; // 初始padding

    // 角色头像拍立得高度（根据showCharacterPhoto参数决定）
    if (showCharacterPhoto) {
  ...
  // 绘制Memo内容
  function drawMemoContent(ctx, memo, width, height, theme, customFont, showCharacterPhoto) {
    return new Promise((resolve) => {
      ...
      // 在外层声明，供异步回调与 continueDrawing 使用
      let displayNameText = '';
      ...
      try {
        ...
        displayNameText = `${userName} & ${charName}`;
        ...
        // 拍立得效果 - 角色和用户头像并排
        const polaroidWidth = 100;   // 拍立得宽度（稍微缩小以容纳两张）
        const polaroidHeight = 150;  // 拍立得高度（稍微缩小）
        const whiteBorder = 8;       // 白色边框宽度（稍微缩小）
        const polaroidSpacing = 20;  // 两张拍立得之间的间距
        
        // 角色拍立得位置（左侧，向左倾斜）
        const charPolaroidX = padding + 10;
        const charPolaroidY = currentY;
        
        // 用户拍立得位置（右侧，向右倾斜）
        const userPolaroidX = charPolaroidX + polaroidWidth + polaroidSpacing;
        const userPolaroidY = currentY + 10; // 稍微错开一点Y位置
        ...
        // 绘制角色拍立得（向左倾斜 -5度）
        ctx.save();
        ctx.translate(charPolaroidX + polaroidWidth/2, charPolaroidY + polaroidHeight/2);
        ...
        ctx.fillRect(charPolaroidX, charPolaroidY, polaroidWidth, polaroidHeight);
        
        // 移除阴影效果，绘制角色照片区域
        ctx.shadowColor = 'transparent';
        
        // 定义角色照片区域
        const charPhotoWidth = polaroidWidth - whiteBorder * 2;
        const charPhotoHeight = polaroidHeight - whiteBorder * 2 - 18; // 留出18px的空间放名字
        const charPhotoX = charPolaroidX + whiteBorder;
        const charPhotoY = charPolaroidY + whiteBorder;
        
        // 绘制角色照片背景（灰色占位）
        ctx.fillStyle = '#f0f0f0';
        ctx.fillRect(charPhotoX, charPhotoY, charPhotoWidth, charPhotoHeight);
        
        ctx.restore(); // 恢复角色拍立得的变换
        
        // 绘制用户拍立得（向右倾斜 +3度）
        ctx.save();
        ctx.translate(userPolaroidX + polaroidWidth/2, userPolaroidY + polaroidHeight/2);
        ...
        ctx.shadowOffsetY = 2;
        ctx.fillStyle = '#ffffff';
        ctx.fillRect(userPolaroidX, userPolaroidY, polaroidWidth, polaroidHeight);
        
        // 移除阴影效果，绘制用户照片区域
        ctx.shadowColor = 'transparent';
        
        // 定义用户照片区域
        const userPhotoWidth = polaroidWidth - whiteBorder * 2;
        const userPhotoHeight = polaroidHeight - whiteBorder * 2 - 18; // 留出18px的空间放名字
        const userPhotoX = userPolaroidX + whiteBorder;
        const userPhotoY = userPolaroidY + whiteBorder;
        
        // 绘制用户照片背景（灰色占位）
        ctx.fillStyle = '#f0f0f0';
        ctx.fillRect(userPhotoX, userPhotoY, userPhotoWidth, userPhotoHeight);
        
        ctx.restore(); // 恢复用户拍立得的变换
        ...
        if (window.parent && window.parent.TavernHelper) {
          try {
            // 获取角色头像
            if (window.parent.TavernHelper.getCharAvatarPath) {
        ...
        // 尝试通过酒馆宏获取用户头像
        function getUserAvatarFromMacro() {
          try {
            // 检查是否有TavernHelper和宏替换功能
            if (window.parent && window.parent.TavernHelper && window.parent.TavernHelper.substitudeMacros) {
              const userAvatarPath = window.parent.TavernHelper.substitudeMacros('{{useravatarpath}}');
              
              // 检查宏是否被成功替换（不是原始宏字符串）
              if (userAvatarPath && userAvatarPath !== '{{useravatarpath}}' && userAvatarPath.trim() !== '') {
                console.log('Memo: 通过宏找到用户头像路径:', userAvatarPath);
                ...
                if (userAvatarPath.startsWith('./')) {
                  // 使用父窗口的URL作为基础URL
                  const baseUrl = window.parent.location.origin + window.parent.location.pathname.replace(/\/[^\/]*$/, '/');
                  const fullUrl = new URL(userAvatarPath, baseUrl).href;
                  console.log('Memo: 转换为绝对路径:', fullUrl);
                  return fullUrl;
                } else if (userAvatarPath.startsWith('/')) {
                  // 绝对路径，添加父窗口的origin
                  return window.parent.location.origin + userAvatarPath;
                } else {
                  // 已经是完整URL
                  ...
                  // 绝对路径，添加origin
                  return window.location.origin + userAvatarPath;
                } else {
                  // 已经是完整URL
                  return userAvatarPath;
                }
            ...
            console.warn('Memo: 无法通过宏获取用户头像');
            return null;
          } catch (e) {
            console.error('Memo: 通过宏获取用户头像失败:', e);
            return null;
          }
        }
        
        userImageUrl = getUserAvatarFromMacro();
        ...
        if (charImageUrl) {
          ...
          charImg.onload = function() {
            ...
            const areaRatio = charPhotoWidth / charPhotoHeight;
            
            let scale;
            if (imgRatio <= areaRatio) {
              scale = charPhotoHeight / charImg.height;
            } else {
              scale = charPhotoWidth / charImg.width;
            }
            
            const drawWidth = charImg.width * scale;
            const drawHeight = charImg.height * scale;
            const offsetX = (charPhotoWidth - drawWidth) / 2;
            const offsetY = (charPhotoHeight - drawHeight) / 2;
        ...
        if (userImageUrl) {
          ...
          userImg.onload = function() {
            ...
            const areaRatio = userPhotoWidth / userPhotoHeight;
            
            let scale;
            if (imgRatio <= areaRatio) {
              scale = userPhotoHeight / userImg.height;
            } else {
              scale = userPhotoWidth / userImg.width;
            }
            
            const drawWidth = userImg.width * scale;
            const drawHeight = userImg.height * scale;
            const offsetX = (userPhotoWidth - drawWidth) / 2;
            const offsetY = (userPhotoHeight - drawHeight) / 2;
...
// 获取是否显示角色照片的状态
const showCharacterPhotoCheckbox = modalBodyElement.querySelector('#showCharacterPhoto');
const showCharacterPhoto = showCharacterPhotoCheckbox ? showCharacterPhotoCheckbox.checked : true;

regenerateImageWithNewSettings(memo, newStyle, state.fontConfig.currentFont, previewImage, loadingIndicator, styleSelector, fontSelector, showCharacterPhoto);
});
    ...
    fontSelector.addEventListener('change', (e) => {
      ...
      regenerateImageWithNewSettings(memo, currentStyleValue, newFont, previewImage, loadingIndicator, styleSelector, fontSelector, showCharacterPhoto);
    });

    // 角色照片复选框事件
    const showCharacterPhotoCheckbox = modalBodyElement.querySelector('#showCharacterPhoto');
    if (showCharacterPhotoCheckbox) {
      showCharacterPhotoCheckbox.addEventListener('change', (e) => {
        const showCharacterPhoto = e.target.checked;
        const currentStyleValue = styleSelector.value;
        const currentFontValue = fontSelector.value;
        
        regenerateImageWithNewSettings(memo, currentStyleValue, currentFontValue, previewImage, loadingIndicator, styleSelector, fontSelector, showCharacterPhoto);
      });
    }
    ...
    if (regenerateBtn) {
      regenerateBtn.addEventListener('click', () => {
        ...
        regenerateImageWithNewSettings(memo, selectedStyle, selectedFont, previewImage, loadingIndicator, styleSelector, fontSelector, showCharacterPhoto);
      });
    }
  ...
  // 重新生成图片
  function regenerateImageWithNewSettings(memo, style, font, previewImage, loadingIndicator, styleSelector, fontSelector, showCharacterPhoto) {
    // 显示加载指示器
    ...
    // 生成新图片
    generateMemoImage(memo, style, font, showCharacterPhoto).then(newImageDataUrl => {
